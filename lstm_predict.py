...
def fund_catagory():#将基金按照ID分开成222个基金
    ...
    return fund
data=fund_catagory()[0]['close']
#normalize_data=(data-np.mean(data))/np.std(data)  #标准化
normalize_data=np.array(data)
    ...
    with tf.Session() as sess:
        ...
        for i in range(10):
            ...
            while(end<len(train_x)):
                ...
                if step%10==0:
                    print(i,step,loss_)
                    print("保存模型：",saver.save(sess,'./lstm_model/stock.model'))
                step+=1
train_lstm()
#————————————————预测模型————————————————————
def prediction():
    ...
    with tf.Session() as sess:
        ...
        #预测未来一周5天的股价
        for i in range(5):
            next_seq=sess.run(pred,feed_dict={X:[prev_seq]})    #长度为20
        ...
        plt.show()
        # print('next_seq的长度为', len(next_seq))
        # print('predict的长度为', len(predict))
